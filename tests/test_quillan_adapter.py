...
def test_descriptor_is_exact_released_contract() -> None:
    ...
    assert QUILLAN_READER_VERSION == "0.10.1"
    ...
    assert descriptor.supported_producer_reader_versions == frozenset({"0.10.1"})
...
@pytest.mark.parametrize("version", ["0.9.0", "0.10.0", "0.10.2"])
...
def test_projection_preserves_order_states_scale_and_missingness() -> None:
    ...
    registry = AdapterRegistry((QuillanAcademicResultAdapter(),))
    first = registry.invoke(request, lambda _: "0.10.1")
    second = registry.invoke(request, lambda _: "0.10.1")
    assert first == second
...
def test_targets_and_provenance_preserve_native_semantics_without_text() -> None:
    inventory = AdapterRegistry((QuillanAcademicResultAdapter(),)).invoke(
        projection_request(), lambda _: "0.10.1"
    )
    observation = next(
        item
        for item in inventory.items
