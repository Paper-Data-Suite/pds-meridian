...
def _selection_preview() -> object:
    ...
    return SimpleNamespace(
        ...
        target_applicability=SimpleNamespace(status="current", reason_codes=()),
        class_id=CLASS_ID,
        derivation_id=DERIVATION_ID,
        target_review_revision=1,
        target_review_sha256=REVIEW_SHA256,
...
def _selection_result() -> object:
    return SimpleNamespace(
        ...
        previous_current_review_revision=None,
        selected_review_revision=1,
        selected_review_sha256=REVIEW_SHA256,
        selected_decision="accepted_for_export",
        review_write_action="not_performed",
        review_selection_action="performed",
        core_export_action="not_performed",
        csv_export_action="not_performed",
    )


def test_review_selection_default_is_read_only_cas_preview(
    ...
    monkeypatch.setattr(cli, "preview_planning_signal_review_selection", build)
    monkeypatch.setattr(
        cli,
        "commit_planning_signal_review_selection",
        lambda *args, **kwargs: pytest.fail("default selection mode must not mutate"),
    )
    monkeypatch.setattr(
        cli,
    ...
    assert cli.main(_selection_args()) == 0
    ...
    assert "Create Planning Signal — #39 review selection" in output
    assert "CANONICAL #39 TEACHER PROJECTION" in output
    assert "target review revision: 1" in output
    assert "target decision: accepted_for_export" in output
    assert "target applicability: current" in output
    assert "review selection confirmation supplied: no" in output
    assert "NO REVIEW SELECTION CHANGED" in output
    assert "NO CORE GROUPING SIGNAL OR CSV EXPORTED" in output


def test_confirm_review_selection_commits_only_selector(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    preview = _selection_preview()
    ...
    monkeypatch.setattr(cli, "commit_planning_signal_review_selection", commit)
    monkeypatch.setattr(
        cli,
        "format_grouping_signal_teacher_projection",
        lambda value: "CANONICAL #39 TEACHER PROJECTION",
    ...
    assert cli.main(_selection_args("--confirm-review-select")) == 0
    output = capsys.readouterr().out

    assert observed == [(('synthetic-workspace', preview), {})]
    assert "review selection confirmation supplied: yes" in output
    assert "#39 review selected: revision 1 (created)" in output
    assert f"selected review SHA-256: {REVIEW_SHA256}" in output
    assert "NO TEACHER REVIEW WRITTEN BY THIS COMMAND" in output
    assert "NO CORE GROUPING SIGNAL OR CSV EXPORTED" in output


def test_confirmed_selection_json_preserves_export_boundary(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    monkeypatch.setattr(
        cli,
        "preview_planning_signal_review_selection",
        lambda *args, **kwargs: _selection_preview(),
    )
    monkeypatch.setattr(
        cli,
        "commit_planning_signal_review_selection",
        lambda *args, **kwargs: _selection_result(),
    )

    assert cli.main(
        _selection_args("--confirm-review-select", "--format", "json")
    ) == 0
    data = json.loads(capsys.readouterr().out)

    assert data["task"] == "create-planning-signal"
    assert data["mode"] == "review_selected"
    assert data["review_selection_confirmed"] is True
    assert data["selection"]["selected_review_revision"] == 1
    assert data["selection"]["selected_review_sha256"] == REVIEW_SHA256
    assert data["selection"]["selected_decision"] == "accepted_for_export"
    assert data["actions"]["review_write"] == "not_performed"
    assert data["actions"]["review_selection"] == "performed"
...
def test_review_selection_target_arguments_must_be_paired(
    capsys: pytest.CaptureFixture[str],
) -> None:
    assert cli.main(
        _review_source_args("--select-review-revision", "1")
    ) == 1
    error = capsys.readouterr().err
    assert "review_selection_invalid" in error
    assert "requires both --select-review-revision" in error


def test_review_selection_requires_exact_preview_source(
    capsys: pytest.CaptureFixture[str],
) -> None:
    assert cli.main(
        _arguments(
...
def test_review_selection_cannot_be_combined_with_review_authoring(
    capsys: pytest.CaptureFixture[str],
) -> None:
    assert cli.main(
        _selection_args(
...
def test_confirm_review_select_requires_exact_selection_target(
    capsys: pytest.CaptureFixture[str],
) -> None:
    assert cli.main(
        _review_source_args("--confirm-review-select")
    ) == 1
    error = capsys.readouterr().err
    assert "review_selection_invalid" in error
    assert "requires an exact persisted #39 review target" in error
