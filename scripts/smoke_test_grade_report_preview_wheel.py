"""Run issue #54's installed Grade/report preview acceptance."""
...
PROGRAM = Path(__file__).with_name("smoke_program_grade_report_preview.py")
RELOAD_PROGRAM = Path(__file__).with_name(
    "smoke_program_grade_report_preview_reload.py"
)
...
def run_prepared_smoke(python: Path, outside: Path) -> None:
    """Run Grade/report-preview acceptance in a prepared environment."""
...
def smoke_test(
    ...
    with tempfile.TemporaryDirectory(
        prefix="pds-meridian-grade-preview-smoke-"
    ) as raw_temp:
        root = Path(raw_temp)
