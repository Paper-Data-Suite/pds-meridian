"""Smoke-test #39 preview/review from an isolated installed Meridian wheel."""
...
def smoke_test(meridian_wheel: Path, core_wheel: Path) -> None:
    """Install exact Core + Meridian and exercise the #39 vertical path."""

    with tempfile.TemporaryDirectory(
        prefix="pds-meridian-grouping-signal-preview-review-smoke-"
    ) as raw_temp:
        root = Path(raw_temp)
...
def main(argv: list[str] | None = None) -> int:
    """Parse wheel paths and run the isolated #39 installed-wheel smoke."""
