"""Smoke-test #40 export from an isolated installed Meridian wheel."""
...
PROGRAM = Path(__file__).with_name("smoke_program_grouping_signal_export.py")
...
def smoke_test(meridian_wheel: Path, core_wheel: Path) -> None:
    """Install exact Core + Meridian and exercise the #40 vertical path."""

    with tempfile.TemporaryDirectory(
        prefix="pds-meridian-grouping-signal-export-smoke-"
    ) as raw_temp:
        root = Path(raw_temp)
...
def main(argv: list[str] | None = None) -> int:
    """Parse wheel paths and run the isolated #40 installed-wheel smoke."""
