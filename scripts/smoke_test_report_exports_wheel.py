"""Run issue #56's installed report-export acceptance."""
...
SNAPSHOT_PROGRAM = Path(__file__).with_name(
    "smoke_program_reporting_snapshot.py"
)
PROGRAM = Path(__file__).with_name("smoke_program_report_exports.py")
RELOAD_PROGRAM = Path(__file__).with_name(
    "smoke_program_report_exports_reload.py"
)
...
def run_prepared_smoke(python: Path, outside: Path) -> None:
    """Run report-export acceptance in a prepared environment."""
    _run(
        [str(python), "-m", "meridian", "reporting", "exports", "--help"],
        outside,
    )
    _run([str(python), str(SNAPSHOT_PROGRAM.resolve()), str(outside)], outside)
...
def smoke_test(
    ...
    """Install released dependencies plus the candidate Meridian wheel."""

    with tempfile.TemporaryDirectory(
        prefix="pds-meridian-report-export-smoke-"
    ) as raw_temp:
        root = Path(raw_temp)
